...
def made_animation(coord_x, coord_y, folder_path, size, life_time):
    ...
    fire_smena = pygame.event.custom_type()
    pygame.time.set_timer(fire_smena, 100)
    slovar_animation = {"coord": [coord_x, coord_y], "image_number": 0, "folder_path": folder_path, "size": size,
                        "life_time": life_time, "active": True, "fire_potux_timer": fire_potux, "fire_smena_timer":fire_smena}
...
def _animation(animation):  # model
