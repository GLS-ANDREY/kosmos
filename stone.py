...
def made_stone(x_stone, y_stone):
    ...
    size_stone = stone_slovar["rotate"].get_size()
...
def polet_stone(stone):
    #TODO: В начале урока запускаешь программу и смотришь на камни - они движутся рывками P.s Вячеслав тоже не знает почему
    ...
    center_rotate = [stone["coord"][0] + size_stone[0] / 2, stone["coord"][1] + size_stone[1] / 2]
    center_rotate[0] -= stone["center"][0]
    center_rotate[1] -= stone["center"][1]

    stone["coord_draw"][0] = stone["coord"][0] - center_rotate[0]
    stone["coord_draw"][1] = stone["coord"][1] - center_rotate[1]
